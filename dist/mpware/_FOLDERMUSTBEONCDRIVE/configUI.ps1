...
if (Test-Path "$env:USERPROFILE\mpware-config.cfg" -ErrorAction SilentlyContinue) {
    $dconfigTextbox.Text = "$env:USERPROFILE\mpware-config.cfg"
}
else {
...
$exportConfig = Create-ModernButton -Text 'Export Tweaks' -Location (New-Object Drawing.Point(250, 190)) -Size (New-Object Drawing.Size(120, 30)) -borderSize 2 -ClickAction {
    if (Test-Path "$env:USERPROFILE\mpware-config.cfg" -ErrorAction SilentlyContinue) {
        ...
        if ($folderDialog.ShowDialog() -eq 'OK') {
            ...
            Copy-Item -Path "$env:USERPROFILE\mpware-config.cfg" -Destination $selectedFolder -Force
            $date = Get-Date -Format 'MM-dd-yy'
            Rename-Item -Path "$selectedFolder\mpware-config.cfg" -NewName "mpware-config$($date).cfg" -Force
