...
if ($offlineMode) {
    $host.ui.RawUI.WindowTitle = 'mpware ', $currentVersion, '(OFFLINE)'
}
else {
    $host.ui.RawUI.WindowTitle = 'mpware ', $currentVersion  
}
...
$fullTitle = '~' * [Math]::Floor($padding) + ' ' + 'WELCOME TO mpware' + ' ' + '~' * [Math]::Ceiling($padding)
...
if (Test-Path "$folder\$currentVersion" -PathType Leaf) {
    ...
    Write-Host "Make sure you do not have older versions of mpware on $sysDrive"
    Write-Host 'Press any Key to Exit: '
    $null = $Host.UI.RawUI.ReadKey('NoEcho,IncludeKeyDown')
    exit
}
...
opDarkAccents = 0
opNoMouseAccel = 0
opDisableDeviceEncryption = 0
opClassicAccents = 0
opStartMenuShortcutClean = 0
opStartMenuShortcuts = 0
#CONTEXT MENU TWEAKS
connewFiles = 0
conmorePS = 0
consnipping = 0
...
if (!(Test-Path -Path $config)) {
    ...
    foreach ($line in $currentContent) {
        if ($line -notlike '#*' -and !([string]::IsNullOrWhiteSpace($line))) {
            ...
            $currentConfigNames += $settingName
