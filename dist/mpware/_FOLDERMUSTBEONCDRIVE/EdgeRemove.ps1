...
if ($OS.BuildNumber -gt 19045) {
    ...
    Set-ItemProperty -Path $NationPath -Name 'Nation' -Value 244 -Force
    Set-ItemProperty -Path $NationPath -Name 'Name' -Value 'US' -Force
}
...
#remove experiment control labels that could block uninstall
Remove-ItemProperty 'HKLM:\SOFTWARE\Microsoft\EdgeUpdate\ClientState\{56EB18F8-B008-4CBD-B6D2-8C97FE7E9062}' -Name 'experiment_control_labels' -Force -ErrorAction SilentlyContinue
Remove-ItemProperty 'HKLM:\SOFTWARE\WOW6432Node\Microsoft\EdgeUpdate\ClientState\{56EB18F8-B008-4CBD-B6D2-8C97FE7E9062}' -Name 'experiment_control_labels' -Force -ErrorAction SilentlyContinue

#edge uninstall string needs to be ran from cmd in a temp path to allow uninstall in some regions
#credit: https://gist.github.com/ave9858/c3451d9f452389ac7607c99d45edecc6
...
if ($uninstallString) {
    $uninstallString = $uninstallString + ' --force-uninstall'
    Start-Process -FilePath $fakeDllhostPath -ArgumentList "/c $uninstallString" -Wait -WindowStyle Hidden
}
else {
...
$removelockedFunc = "$PSScriptRoot\Remove-Locked.ps1"

foreach ($package in $remove_appx) {
    Run-Trusted -command ".'$removelockedFunc'; Remove-Locked $package"
    Start-Sleep 1
}

#remove active setup reg keys if they are there
$path1 = 'HKLM:\SOFTWARE\WOW6432Node\Microsoft\Active Setup\Installed Components'
$path2 = 'HKLM:\SOFTWARE\Microsoft\Active Setup\Installed Components'

$keys = Get-ChildItem $path1, $path2
foreach ($key in $keys) {
    $item = Get-ItemProperty $key.pspath
    if ($item.stubpath -like '*Edge*') {
...
Remove-Item $fakeDllhostPath -ErrorAction SilentlyContinue

Reg.exe delete 'HKLM\SOFTWARE\Microsoft\WindowsUpdate\Orchestrator\UScheduler_Oobe\EdgeUpdate' /f *>$null
