...
if (Check-Internet) {
    ...
    $label3.Location = New-Object System.Drawing.Point(240, 343)
    $label3.Size = New-Object System.Drawing.Size(130, 30)
    ...
    $textboxCustom2.Add_TextChanged({
            if ($textboxCustom2.Text -ne '') {
                $comboBox.SelectedIndex = -1
            }
        })

    $tooltip2 = New-Object System.Windows.Forms.ToolTip
    $tooltip2.SetToolTip($textboxCustom2, 'Select your own driver file instead')

    $filebrowsebttn = New-Object System.Windows.Forms.Button
    $filebrowsebttn.Location = New-Object System.Drawing.Point(270, 380)
    $filebrowsebttn.Size = New-Object System.Drawing.Size(40, 23)
    $filebrowsebttn.Text = '...'
    ...
    $filebrowsebttn.Add_Click({    
            ...
            if ($driverFile) {
                $textboxCustom2.Text = $driverFile
            }
        })
    ...
    if ($result -eq [System.Windows.Forms.DialogResult]::OK) {
        ...
        elseif ($textboxCustom2.Text -ne '' -and (Test-Path $textboxCustom2.Text)) {
            $selectedDriver = $null
            $selectedDriverPath = $textboxCustom2.Text
        }
        else {
        ...
        if ($stripDriver) {
            ...
            if ($driverNum -eq $null -or [version]$driverNum -ge '566.36') {
        ...
        if ($stripDriver) {
            ...
            $files = (Get-ChildItem $pluginPath -Recurse -File).FullName

            if ($files) {
                Stop-Service NVDisplay.ContainerLocalSystem -Force -ErrorAction SilentlyContinue
                foreach ($file in $files) {
                    $fileName = Split-Path $file -Leaf
                    if ($pluginsToRemove -contains $fileName) {
                        takeown /f $file *>$null
                        icacls $file /grant *S-1-5-32-544:F /t *>$null
    ...
    $optionallabel.ForeColor = 'White'
    $optionallabel.Visible = $false
    $optionallabel.BackColor = [System.Drawing.Color]::Transparent
    $optionallabel.AutoSize = $true
    $optionallabel.Font = New-Object System.Drawing.Font('Segoe UI', 9, [System.Drawing.FontStyle]::Bold) 
    $optionallabel.Location = New-Object System.Drawing.Point(10, 180)
    $tabPage1.Controls.Add($optionallabel)

    $textboxCustom = New-Object System.Windows.Forms.TextBox
    $textboxCustom.Location = New-Object System.Drawing.Point(80, 180)
    $textboxCustom.Size = New-Object System.Drawing.Size(180, 20)
    ...
    $filebrowsebttn = New-Object System.Windows.Forms.Button
    $filebrowsebttn.Location = New-Object System.Drawing.Point(270, 180)
    $filebrowsebttn.Size = New-Object System.Drawing.Size(40, 23)
    $filebrowsebttn.Text = '...'
    $filebrowsebttn.Visible = $false
    $filebrowsebttn.Cursor = 'Hand'
    ...
    $filebrowsebttn.Add_Click({    
            ...
            if ($nip) {
                $textboxCustom.Text = $nip
            }
            else {
                $textboxCustom.Text = 'Select Path to .nip'
            }
        })
    $TabPage1.Controls.Add($filebrowsebttn)
    ...
    if ($dialogResult -eq [System.Windows.Forms.DialogResult]::OK) {
        ...
        if ($checkbox1.Checked) {
            ...
            if ($textboxCustom.Text -ne 'Select Path to .nip' -and (Test-Path $textboxCustom.Text)) {
                $stockNip = $textboxCustom.Text
            }
            else {
