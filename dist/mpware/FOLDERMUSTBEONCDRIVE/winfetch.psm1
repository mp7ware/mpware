...
'@
    ...
    function info_title {
        return @{
            title   = ""
            content = "${e}[1;36m{0}${e}[0m@${e}[1;36m{1}${e}[0m" -f [System.Environment]::UserName, $env:COMPUTERNAME
        }
    }
    ...
    $version = "$($reg.CurrentBuildNumber).$($reg.UBR)"
        return @{
            title   = "Kernel"
            content = $version
        }
    ...
    foreach ($item in $config) {
        ...
        foreach ($line in $info) {
            $output = "$e[1;36m$($line["title"])$e[0m"
